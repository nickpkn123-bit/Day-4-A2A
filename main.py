...
MY_AGENT_USERNAME = "nick-agent"  # 👈 CHANGE THIS: Your unique username
MY_AGENT_NAME = "Nick's Personal Agent"      # 👈 CHANGE THIS: Human-readable name
MY_AGENT_DESCRIPTION = "AI agent specialized in research, calculations, and web search"  # 👈 CHANGE THIS
MY_AGENT_PROVIDER = "Nick Pan"        # 👈 CHANGE THIS: Your name
MY_AGENT_PROVIDER_URL = "https://github.com/nickpkn123-bit" # 👈 CHANGE THIS: Your website
...
# Tool 3: Web Search via OpenRouter
        ...
        try:
            ...
            response = httpx.post(
                ...
                json={"model": "openai/gpt-4o-mini", "messages": [
                    {"role": "system", "content": "You are a web search assistant. Answer with factual, up-to-date information."},
...
# Tool 4: Website Search via OpenRouter
...
class OpenRouterWebsiteTool(BaseTool):
    ...
    description: str = "Search and extract information from websites or URLs."
    ...
    def _run(self, url_or_topic: str) -> str:
        try:
            ...
            response = httpx.post(
                ...
                headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
                json={"model": "openai/gpt-4o-mini", "messages": [
                    {"role": "system", "content": "You are a web content specialist. Summarize content from the given URL or topic."},
                    {"role": "user", "content": f"Search and summarize: {url_or_topic}"}
                ], "max_tokens": 500},
                timeout=30.0
            )
            return response.json()["choices"][0]["message"]["content"]
...
# Tool 5: YouTube Search via OpenRouter
...
class OpenRouterVideoSearchTool(BaseTool):
    ...
    description: str = "Search for YouTube video content on any topic."
    ...
    def _run(self, query: str) -> str:
        try:
            ...
            response = httpx.post(
                ...
                headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
                json={"model": "openai/gpt-4o-mini", "messages": [
                    {"role": "system", "content": "You are a YouTube content specialist. Find relevant videos and content for the given query."},
                    {"role": "user", "content": f"Find YouTube videos about: {query}"}
                ], "max_tokens": 400},
                timeout=30.0
            )
            return response.json()["choices"][0]["message"]["content"]
...
video_tool = OpenRouterVideoSearchTool()

# Tool 6: Web Search (optional - requires SERPER_API_KEY)
search_tool = None
